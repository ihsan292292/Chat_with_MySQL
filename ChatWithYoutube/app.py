...
def load_video(url:str) -> str:
    ...
    video_title = re.sub(invalid_chars_regex, '_', yt.title.replace(' ', '_'))
    ...
    try:
        ...
        stream.download(output_path=target_dir, filename=video_filename)
    except Exception as e:
        raise gr.Error(f'Issue in Downloading video: {str(e)}')
...
def process_video(video=None, url=None) -> dict[str, str | list]:
    ...
    print("ivdendeeeeyyyy!!",file_dir)
    model = whisper.load_model("medium.en")
    result = model.transcribe(file_dir,fp16=False)
    print("resultttttttttttttttttttt",result['text'])
...
def embed_yt(yt_link: str):
    ...
    url_m = make_chain(url=yt_link)
    print("iiiiiiiii",url_m)
    print("ffffffff",url)
